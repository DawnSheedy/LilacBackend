...
export const checkAndUpdateServerRelations = async (server: DiscordServer, user: DiscordUser) => {
    let association = await UserServer.findOne({ where: { DiscordUserId: user.id, DiscordServerServerId: server.serverId}})
    ...
    if (currentDate - lastPermissionCheck > 60*60*1000) {
        const userIsAdmin = await checkAdminPermissions(user.id, server.serverId)
